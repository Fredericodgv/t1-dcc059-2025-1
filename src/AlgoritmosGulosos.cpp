    ...
    std::vector<char> AlgoritmosGulosos::executar_randomizado_n_vezes(Grafo* grafo, int n_iteracoes, float alfa)
    {
        ...
        for (int i = 0; i < n_iteracoes; ++i)
        {
            ...
            if (melhor_solucao_encontrada.empty() || solucao_atual.size() < melhor_solucao_encontrada.size())
            {
                ...
                std::cout << "  Iteracao " << (i + 1) << ": Nova melhor solucao encontrada com tamanho " << melhor_solucao_encontrada.size() << std::endl;
            }
            else
            {
                solucao_atual.clear();
        ...
        std::cout << "Execucao finalizada. Melhor tamanho de conjunto dominante encontrado: " << melhor_solucao_encontrada.size() << std::endl;
